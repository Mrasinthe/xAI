...
async def explain_api(file: UploadFile = File(...), mlModel: UploadFile = File(...), ImageFileBytes: bytes = File(...)):
    ...
    explaination, top_T, top_T_plot_image, segments, bar_plot_image, segment_overlay_array, pred, lime, segment__img, top_labels_names, scores = explain_lime(
        image)  # loaded_model ,bar_plot_segments,bar_plot_importance
    ...
    return {"top_T": top_T, "image_base64": image_base64, "segments": segments, "bar_plot_base64": bar_plot_base64,
            "segment_overlay_base64": segment_overlay_base64, "top_T_plot_base64": top_T_plot_base64, "pred": pred,
            "top_labels_names": top_labels_names, "scores": scores}
...
async def explain_api(file: UploadFile = File(...), base: ClassLabel = Depends(), mlModel: UploadFile = File(...),
                      ImageFileBytes: bytes = File(...)):
    extension = file.filename.split(".")[-1] in ("jpg", "jpeg", "png")
